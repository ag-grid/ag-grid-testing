...
export async function waitForGridReady(page: Page) {
  await page
    .locator("ag-overlay-loading-center")
    .first()
...
export async function clickAllButtons(page: Page) {
  ...
  const buttons = page.locator(
    "button:visible:not([disabled]):not(.ag-root-wrapper button):not(.ag-chart button)"
  );
...
export async function getRowCountOrError(page: Page): Promise<number | string> {
  ...
  const headerOrError = page
    .locator(".ag-header .ag-header-row")
    .or(page.locator(".example-error"))
    .first();
  ...
  const totalAriaRowCount = await gridLocator
    .first()
...
export function getHeaderLocator(page: Page, options: ColumnLocatorOptions) {
  ...
  return page
    ...
    .and(page.locator(`[col-id="${options.colId}"]`));
}
 ...
 */
export async function getCellLocatorBy(
  page: Page,
  options: CellLocatorOptions
) {
